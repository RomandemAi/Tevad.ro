...
The exact **system prompt** (same text for Anthropic and xAI) is public and version-controlled in [`prompts/neutrality-system-prompt.md`](prompts/neutrality-system-prompt.md) (current line: **v1.4.0 “AI transparency”**).
...
**Multi-model cross-check (v1.4.0):** Every verification runs **Claude Sonnet**, **Claude Haiku**, and **xAI Grok** in parallel (production requires **`XAI_API_KEY`** alongside Anthropic). Each model must return **valid strict JSON** including **`plain_summary`** and **`ai_explain`** (Romanian); malformed or instruction-violating output is treated as a safety signal and can force **PENDING**.
