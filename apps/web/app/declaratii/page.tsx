...
function statementsHref(opts: { status?: RecordStatusFilter | null }) {
  const q = new URLSearchParams()
  if (opts.status) q.set(STATUS_QUERY_PARAM, opts.status)
  const qs = q.toString()
  return qs ? `/declaratii?${qs}` : '/declaratii'
}
...
export const metadata: Metadata = {
  title: 'Declarații',
  description:
    'Declarații publice atribuite politicienilor monitorizați în România — status de verificare, surse citate și nivel de impact.',
  alternates: { canonical: `${getSiteUrl()}/declaratii` },
}
...
export default async function DeclaratiiPage({ searchParams }: Props) {
  ...
  const breadcrumb = (
    <>
      <span className="text-[var(--gray-500)]">TEVAD.ORG /</span> DECLARAȚII
    </>
  )
    ...
    <AppShell
      ...
      topBarRight={
        <span className="font-mono text-[10px] text-[var(--gray-500)]">
          ...
          {statusFilter ? ` (status: ${verdictBadgeLabel('statement', statusFilter)})` : ''}
        </span>
      }
    >
        ...
        <div className="mx-auto max-w-[860px] space-y-3">
          {records.map((rec, i) => {
            ...
            return (
              ...
              >
                <Link href={`/politician/${pol?.slug}`} className="group flex min-w-0 flex-1 items-start gap-3 p-4">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="mt-2 flex flex-wrap items-center gap-2">
                      <span className={`rounded border px-1.5 py-0.5 font-mono text-[8px] ${STATUS_CLASS[rec.status]}`}>
                        {verdictBadgeLabel('statement', rec.status as 'true' | 'false' | 'partial' | 'pending')}
                      </span>
